...
void compute_energy_density(
    polyscope::SurfaceMesh *rest_surface_mesh,
    ...
    int matid)
{
    ...
    std::vector<double> bending_energy_density = LibShell::ElasticShell<SFF>::elasticEnergyPerElement(mesh, curPos, init_edgeDOFs, *mat, restState,
        LibShell::ElasticShell<SFF>::ET_BENDING);
...
int main(int argc, char** argv)
{
    ...
    CLI::App app{argv[0]};
    app.add_option("--rest-mesh", args.input_rest_mesh, "Input Rest State")
        ->required()
    ...
    polyscope::state::userCallback = [&]()
    {
        ...
        if (ImGui::Button("Draw Bending Density", ImVec2(-1,0)))
        {
            ...
            {
            case 0:
                compute_energy_density<LibShell::MidedgeAngleTanFormulation>(rest_surf, cur_surf, mesh, rest_V, cur_V, thickness,
                    lame_alpha, lame_beta, matid);
                break;
            case 1:
                compute_energy_density<LibShell::MidedgeAngleSinFormulation>(rest_surf, cur_surf, mesh, rest_V, cur_V, thickness,
                    lame_alpha, lame_beta, matid);
                break;
            case 2:
                compute_energy_density<LibShell::MidedgeAverageFormulation>(rest_surf, cur_surf, mesh, rest_V, cur_V, thickness,
                    lame_alpha, lame_beta, matid);
                break;
            default:
                assert(false);
