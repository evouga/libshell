...
#include "make_geometric_shapes/Plane.h"
...
void optimizeFullDOFs(ShellEnergy& energy,
    ...
    // compute the vertex mass
    std::vector<double> vertex_mass(cur_pos.rows(), 0);
    for (int i = 0; i < cur_mesh.nFaces(); i++) {
        // compute the face area
        Eigen::Vector3d v0 = cur_pos.row(cur_mesh.faceVertex(i, 0));
        Eigen::Vector3d v1 = cur_pos.row(cur_mesh.faceVertex(i, 1));
        Eigen::Vector3d v2 = cur_pos.row(cur_mesh.faceVertex(i, 2));
        double face_area = (v1 - v0).cross(v2 - v0).norm() / 2;

        for (int j = 0; j < 3; j++) {
            int vid = cur_mesh.faceVertex(i, j);
            vertex_mass[vid] += face_area * density / 3;
        }
    }
    ...
    auto obj_func = [&](const Eigen::VectorXd& var, Eigen::VectorXd* grad, Eigen::SparseMatrix<double>* hessian,
        ...
        for (int i = 0; i < pos.rows(); i++) {
            total_energy -= vertex_mass[i] * 9.81 * pos(i, 1);
        }

        if (grad) {
            for (int i = 0; i < pos.rows(); i++) {
                (*grad)(i * 3 + 1) -= vertex_mass[i] * 9.81;
            }
            *grad = P * (*grad);
        }
...
void processFullModelOneStep(const LibShell::MeshConnectivity& rest_mesh,
    ...
    // ping the top left and bottom right vertices
    double min_x = std::numeric_limits<double>::infinity();
    double min_y = std::numeric_limits<double>::infinity();
    double max_x = -std::numeric_limits<double>::infinity();

    for (int i = 0; i < cur_pos.rows(); i++) {
        if (cur_pos(i, 0) < min_x) {
            min_x = cur_pos(i, 0);
        }
        if (cur_pos(i, 0) > max_x) {
            max_x = cur_pos(i, 0);
        }
        if (cur_pos(i, 1) < min_y) {
            min_y = cur_pos(i, 1);
        }
    }
    ...
    for (int i = 0; i < cur_mesh.nEdges(); i++) {
        if (cur_mesh.edgeFace(i, 0) == -1 || cur_mesh.edgeFace(i, 1) == -1) {
            for (int k = 0; k < 2; k++) {
                ...
                if ((std::abs(cur_pos(vid, 0) - min_x) < 1e-6 && std::abs(cur_pos(vid, 1) - min_y) < 1e-6) ||
                    (std::abs(cur_pos(vid, 0) - max_x) < 1e-6 && std::abs(cur_pos(vid, 1) - min_y) < 1e-6)) {
                    for (int j = 0; j < 3; j++) {
    ...
    std::cout << "cur_bnd_condition: " << cur_bnd_condition.size() << std::endl;
    ...
    optimizeFullDOFs(*stvk_dir_energy_model, rest_state.abars, cur_mesh, cur_pos, cur_edge_dofs, density,
...
void update_rendering(polyscope::SurfaceMesh* cur_surface_mesh,
    ...
    switch (model_type) {
        case SFFModelType::kS1Sin: {
            std::shared_ptr<StVKS1DirectorSinShellEnergy> subPtr =
                std::static_pointer_cast<StVKS1DirectorSinShellEnergy>(stvk_dir_energy_model);
            for (int i = 0; i < cur_mesh.nFaces(); i++) {
                double s1_bending =
                    subPtr->mat_.bendingEnergy(cur_mesh, cur_pos, edge_dofs, rest_state, i, nullptr, nullptr);
                bending_scalars.push_back(s1_bending);
            }
            break;
        }
        case SFFModelType::kS2Sin: {
            std::shared_ptr<StVKS1DirectorTanShellEnergy> subPtr =
                std::static_pointer_cast<StVKS1DirectorTanShellEnergy>(stvk_dir_energy_model);
            for (int i = 0; i < cur_mesh.nFaces(); i++) {
                double s1_bending =
                    subPtr->mat_.bendingEnergy(cur_mesh, cur_pos, edge_dofs, rest_state, i, nullptr, nullptr);
                bending_scalars.push_back(s1_bending);
...
int main(int argc, char* argv[]) {
    ...
    app.add_option("--density", args.density, "Density of the material");
    ...
    Eigen::MatrixXd flatV, curV;
    Eigen::MatrixXi F, curF;
    makePlane(!args.delaunlay_mesh, 1, 1, triangle_area, flatV, F);
    curV = flatV;
    curF = F;
    ...
    auto initialize_all = [&]() {
        ...
        cur_mesh = LibShell::MeshConnectivity(curF);
    ...
    auto initialize_rendering = [&]() {
        surface_mesh = polyscope::registerSurfaceMesh(prefix_name + "Current mesh", curV, curF);
        init_surface_mesh = polyscope::registerSurfaceMesh(prefix_name + "Initial mesh", flatV, F);
        ...
        for (int i = 0; i < cur_mesh.nFaces(); i++) {
            for (int j = 0; j < 3; j++) {
                int eid = cur_mesh.faceEdge(i, j);
                Eigen::Vector3d midpt =
                    (curV.row(cur_mesh.edgeVertex(eid, 0)) + curV.row(cur_mesh.edgeVertex(eid, 1))) / 2.0;
                face_edge_midpts.push_back(midpt);
            }
        }
    ...
    update_rendering(init_surface_mesh, init_pt_mesh, dir_energy_model, extra_energy_terms, cur_mesh, rest_state, curV,
                     edge_dofs, sff_type);
    update_rendering(surface_mesh, pt_mesh, dir_energy_model, extra_energy_terms, cur_mesh, rest_state, curV, edge_dofs,
                     sff_type);
    ...
    polyscope::state::userCallback = [&]() {
        ...
        if (ImGui::InputDouble("Young", &young)) {
            if (young <= 0) {
                young = 1e7;
            }
            reinitialization = true;
        ...
        if (ImGui::Combo("Material Type", &args.material, "StVK\0NeoHookean\0")) {
            ...
            update_rendering(init_surface_mesh, init_pt_mesh, dir_energy_model, extra_energy_terms, cur_mesh,
                             rest_state, curV, edge_dofs, sff_type);
            curV = flatV;
        }
        ...
        if (ImGui::Button("Remake Plane", ImVec2(-1, 0))) {
            double actual_triangle_area = triangle_area;
            makePlane(!args.delaunlay_mesh, 1, 1, triangle_area, flatV, F);
            curV = flatV;
            curF = F;
            reinitialization = true;
        ...
        if (ImGui::Button("Optimize", ImVec2(-1, 0))) {
            ...
            processFullModelOneStep(rest_mesh, rest_state, cur_mesh, dir_energy_model, extra_energy_terms, curV,
                                    edge_dofs, sff_type, density);

            update_rendering(surface_mesh, pt_mesh, dir_energy_model, extra_energy_terms, cur_mesh, rest_state, curV,
                             edge_dofs, sff_type);
    ...
    polyscope::view::upDir = polyscope::UpDir::NegYUp;  // set -Y as up direction
