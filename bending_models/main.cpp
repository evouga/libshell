...
void optimizeEdgeDOFs(ShellEnergy& energy,
    ...
    while (true)
    {
        ...
        std::cout << "Force resid now: " << PF.norm() << ", energy: " << origEnergy - penalty << ", penalty: " <<
    penalty << ", reg: " << reg << std::endl; if (PF.norm() < tol) return;
...
Energies measureFoldEnergy(const LibShell::MeshConnectivity& mesh,
    ...
    Eigen::Matrix2d II = LibShell::MidedgeAngleGeneralSinFormulation::secondFundamentalForm(mesh, cur_pos, s2_dir_sin_edge_dofs, 0, nullptr, nullptr);
    ...
    II = LibShell::MidedgeAngleGeneralTanFormulation::secondFundamentalForm(mesh, cur_pos, s2_dir_tan_edge_dofs, 0, nullptr, nullptr);
...
int main(int argc, char* argv[]) {
    ...
    if (curMeshType == MeshType::MT_SPHERE) {
        ...
        Eigen::MatrixXd origV, fold_V;
        Eigen::MatrixXi rest_F;
        gererated_foleded_mesh(1, 1, M_PI * 0.9, origV, rest_F, fold_V);
        LibShell::MeshConnectivity mesh(rest_F);

        igl::writeOBJ("rest_mesh.obj", origV, rest_F);
        igl::writeOBJ("fold_mesh.obj", fold_V, rest_F);
