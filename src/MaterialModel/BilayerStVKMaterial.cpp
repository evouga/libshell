    ...
    if (hessian) {
        ...
        *hessian += crossTermCoeff2 * dA2 * 0.5 * lameAlpha2 * inner4.transpose() * inner2;
        ...
        for (int i = 0; i < 4; ++i)  // iterate over Mainv and abarinv as if they were vectors
        {
            ...
            *hessian += crossTermCoeff1 * dA1 * (0.5 * lameAlpha1 * M1.trace() * abarinv1(i) + lameBeta1 * Mainv1(i)) *
                        augahess[i];
            *hessian += crossTermCoeff2 * dA2 * (0.5 * lameAlpha2 * M2.trace() * abarinv2(i) + lameBeta2 * Mainv2(i)) *
                        augahess[i];
            *hessian += crossTermCoeff1 * dA1 *
                        (0.5 * lameAlpha1 * sigma1.trace() * abarinv1(i) + lameBeta1 * Sainv1(i)) * bhess[i];
            *hessian += crossTermCoeff2 * dA2 *
                        (0.5 * lameAlpha2 * sigma2.trace() * abarinv2(i) + lameBeta2 * Sainv2(i)) * bhess[i];
        }

        Matrix<double, 1, 18 + 3 * nedgedofs> inner001 =
            abarinv1(0, 0) * bderiv.row(0) + abarinv1(0, 1) * bderiv.row(2);
        Matrix<double, 1, 18 + 3 * nedgedofs> inner011 =
            abarinv1(0, 0) * bderiv.row(1) + abarinv1(0, 1) * bderiv.row(3);
        Matrix<double, 1, 18 + 3 * nedgedofs> inner101 =
            abarinv1(1, 0) * bderiv.row(0) + abarinv1(1, 1) * bderiv.row(2);
        Matrix<double, 1, 18 + 3 * nedgedofs> inner111 =
            abarinv1(1, 0) * bderiv.row(1) + abarinv1(1, 1) * bderiv.row(3);
        *hessian += 2 * coeff1 * dA1 * lameBeta1 * inner001.transpose() * inner001;
        *hessian += 2 * coeff1 * dA1 * lameBeta1 * (inner011.transpose() * inner101 + inner101.transpose() * inner011);
        *hessian += 2 * coeff1 * dA1 * lameBeta1 * inner111.transpose() * inner111;

        Matrix<double, 1, 18 + 3 * nedgedofs> inner002 =
            abarinv2(0, 0) * bderiv.row(0) + abarinv2(0, 1) * bderiv.row(2);
        Matrix<double, 1, 18 + 3 * nedgedofs> inner012 =
            abarinv2(0, 0) * bderiv.row(1) + abarinv2(0, 1) * bderiv.row(3);
        Matrix<double, 1, 18 + 3 * nedgedofs> inner102 =
            abarinv2(1, 0) * bderiv.row(0) + abarinv2(1, 1) * bderiv.row(2);
        Matrix<double, 1, 18 + 3 * nedgedofs> inner112 =
            abarinv2(1, 0) * bderiv.row(1) + abarinv2(1, 1) * bderiv.row(3);
        *hessian += 2 * coeff2 * dA2 * lameBeta2 * inner002.transpose() * inner002;
        *hessian += 2 * coeff2 * dA2 * lameBeta2 * (inner012.transpose() * inner102 + inner102.transpose() * inner012);
        *hessian += 2 * coeff2 * dA2 * lameBeta2 * inner112.transpose() * inner112;
        ...
        *hessian +=
            crossTermCoeff1 * dA1 * lameBeta1 * (inner011.transpose() * inner103 + inner103.transpose() * inner011);
        *hessian +=
            crossTermCoeff1 * dA1 * lameBeta1 * (inner013.transpose() * inner101 + inner101.transpose() * inner013);
        ...
        *hessian +=
            crossTermCoeff2 * dA2 * lameBeta2 * (inner012.transpose() * inner104 + inner104.transpose() * inner012);
        *hessian +=
            crossTermCoeff2 * dA2 * lameBeta2 * (inner014.transpose() * inner102 + inner102.transpose() * inner014);
...
template class BilayerStVKMaterial<MidedgeAngleGeneralTanFormulation>;
};  // namespace LibShell