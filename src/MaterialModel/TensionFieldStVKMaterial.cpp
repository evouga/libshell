    ...
    if (puretension) {
        ...
        if (derivative || hessian) {
            ...
            Eigen::Matrix2d mat =
                ...
                sign / denom *
                    (T / 4.0 * abarinv -
                     1.0 / 2.0 * detAbarinv * adjstrain);  // lamda equals (tr(A) + sqrt(tr(A)^2 - 4 * det(A)))/2
            ...
            if (hessian) {
                ...
                //(*hessian) += dA * sign * lambda / denom * (-1.0 / 2.0 / abar.determinant()) *
                ...
                (*hessian) +=
                    2.0 * kstretching * dA * sign * lambda / denom / 4.0 * abarinvterm.transpose() * abarinvterm;
                ...
                (*hessian) +=
                    2.0 * kstretching * -dA * sign * lambda / denom / denom / denom * innerVec.transpose() * innerVec;
            }
        }
        return result;
    }
...
template class TensionFieldStVKMaterial<MidedgeAngleGeneralFormulation>;
template class TensionFieldStVKMaterial<MidedgeAngleGeneralSinFormulation>;
template class TensionFieldStVKMaterial<MidedgeAngleGeneralTanFormulation>;
};  // namespace LibShell